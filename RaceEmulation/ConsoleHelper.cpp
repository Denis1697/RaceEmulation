...
ConsoleHelper::drawWindow(const Coordinate& leftTopCoord,
	...
	cout << static_cast<char>(EnumHelper::eWindowPart::LEFT_TOP_CORNER);
	setCursorPosition({ x2, y1 });
	cout << static_cast<char>(EnumHelper::eWindowPart::RIGHT_TOP_CORNER);
	setCursorPosition(rightBottomCoord);
	cout << static_cast<char>(EnumHelper::eWindowPart::RIGHT_BOTTOM_CORNER);
	setCursorPosition({ x1, y2 });
	cout << static_cast<char>(EnumHelper::eWindowPart::LEFT_BOTTOM_CORNER);


	for (int i = 0; i < dX - 1; i++) {
		setCursorPosition({ x1 + i + 1, y1 });
		cout << static_cast<char>(EnumHelper::eWindowPart::HORIZONTAL_PART);
	...
	for (int i = 0; i < dY - 1; i++) {
		...
		cout << static_cast<char>(EnumHelper::eWindowPart::VERTICAL_PART);
